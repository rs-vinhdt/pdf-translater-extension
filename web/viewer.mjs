...
function showFloatingIcon(rect) {
  ...
  iconImg.src = chrome.runtime.getURL('tooltip-icon.png');
...
function handleIconClick() {
  const iconButton = document.getElementById('my-extension-icon');
  if (!iconButton) return;

  // Show loading animation
  iconButton.innerHTML = `
    <div class="my-loading-dots">
...
function showTooltip(text, rect) {
  ...
  content.className = 'my-tooltip-content';
...
function removeFloatingElements() {
  document.getElementById('my-extension-icon')?.remove();
  document.getElementById('my-extension-tooltip')?.remove();
  document.removeEventListener('mousedown', handleClickOutside, true);
}

// Handles clicking outside to close the tooltip
function handleClickOutside(event) {
  const tooltip = document.getElementById('my-extension-tooltip');
...
function loadTooltipStyles() {
    if (document.getElementById('my-extension-tooltip-styles')) return;
    const link = document.createElement('link');
    link.id = 'my-extension-tooltip-styles';
    link.rel = 'stylesheet';
    link.type = 'text/css';
    link.href = chrome.runtime.getURL('web/tooltip.css');
...
function initializeMyExtension() {
  ...
  viewerContainer.addEventListener('mouseup', (event) => {
    ...
    setTimeout(() => {
        ...
        const clickedOnOurUI = event.target.closest('#my-extension-icon, #my-extension-tooltip');
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === "SHOW_PROCESSED_RESULT") {
      if (lastSelectionRect) {
        showTooltip(request.text, lastSelectionRect);
      }
    }
  });
}
